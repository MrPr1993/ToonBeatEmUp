...
function controller_p3(_controlno){

//var _controlno=1;

key_right = keyboard_check(ord(global.ControlRight[_controlno]));
	key_left = -keyboard_check(ord(global.ControlLeft[_controlno]));
	key_right_pressed = keyboard_check_pressed(ord(global.ControlRight[_controlno]));
	key_left_pressed = -keyboard_check_pressed(ord(global.ControlLeft[_controlno]));
	...
	key_up = keyboard_check(ord(global.ControlUp[_controlno]));
	key_down = keyboard_check(ord(global.ControlDown[_controlno]));
	key_up_pressed = keyboard_check_pressed(ord(global.ControlUp[_controlno]));
	key_down_pressed = -keyboard_check_pressed(ord(global.ControlDown[_controlno]));
...
var i = _controlno;
    ...
    else if (stick_down_held && gamepad_axis_value(i,gp_axislv) > -threshold)
    {
        ...
        keytick=1;
	        ...
	        if keyboard_check(ord(global.ControlRight[_controlno])) or gamepad_axis_value(i, gp_axislh)>0.1 or gamepad_button_check(i,gp_padr)
	       key_right= 1 else key_right=0
	        if keyboard_check(ord(global.ControlLeft[_controlno])) or gamepad_axis_value(i, gp_axislh)<-0.1 or gamepad_button_check(i,gp_padl)
	       key_left= -1 else key_left=0
       
  
	      if keyboard_check_pressed(ord(global.ControlRight[_controlno]))  or gamepad_button_check_pressed(i,gp_padr) or stick_right_pressed
	      key_right_pressed = 1 else  key_right_pressed = 0
	if   keyboard_check_pressed(ord(global.ControlLeft[_controlno]))  or gamepad_button_check_pressed(i,gp_padl) or stick_left_pressed
	    key_left_pressed = -1 else  key_left_pressed = 0
    
	          if keyboard_check_pressed(ord(global.ControlUp[_controlno]))  or gamepad_button_check_pressed(i,gp_padu) or stick_down_pressed
	      key_up_pressed = 1 else  key_up_pressed = 0
	if   keyboard_check_pressed(ord(global.ControlDown[_controlno]))  or gamepad_button_check_pressed(i,gp_padd) or stick_up_pressed
	    key_down_pressed = -1 else  key_down_pressed = 0

	key_up = keyboard_check(ord(global.ControlUp[_controlno])) or gamepad_axis_value(i, gp_axislv)<-0.1 or gamepad_button_check(i,gp_padu)
	key_down = keyboard_check(ord(global.ControlDown[_controlno])) or gamepad_axis_value(i, gp_axislv)>0.9 or gamepad_button_check(i,gp_padd)
