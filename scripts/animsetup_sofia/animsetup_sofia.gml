...
if animFrame>15.5 animFrame=0
	...
	{
	if sprite_index!=spr_sofia_idle
	{animFrame=0 sprite_index=spr_sofia_idle}
	frame_set(0,0,0.1)
...
weaponanim(weaponspr,weaponIndex,21,-40,101*image_xscale,weaponcolor)
	...
	{if key_shield_pressed event_user(4)
	if key_attack {{PlaySound(snd_swing)} comboHit=0 animFrame=0 anim=13 atk=1}}
	hit=0 MoveType=0
	selfatk.recovery=5
	sprite_index=spr_sofia_attack
	image_index=animFrame image_speed=0
	...
	{if key_shield_pressed event_user(4)
	if key_attack {{PlaySound(snd_swing) PlaySound(snd_sofia5)} comboHit=0 animFrame=0 anim=14 atk=1}}
	hit=0 MoveType=0
	selfatk.recovery=10
