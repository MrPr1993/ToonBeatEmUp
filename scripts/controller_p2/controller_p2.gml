...
function controller_p2(_controlno){
...
var i = _controlno;
	...
	if gamepad_button_check_pressed(i,gp_padl) or stick_left_pressed
	    key_left_pressed = -1 else  key_left_pressed = 0
	          ...
	          if gamepad_button_check_pressed(i,gp_padu) or stick_down_pressed
	      key_up_pressed = 1 else  key_up_pressed = 0
	if gamepad_button_check_pressed(i,gp_padd) or stick_up_pressed
	    key_down_pressed = -1 else  key_down_pressed = 0

	key_up =  gamepad_axis_value(i, gp_axislv)<-0.1 or gamepad_button_check(i,gp_padu)
	key_down = gamepad_axis_value(i, gp_axislv)>0.9 or gamepad_button_check(i,gp_padd)
...
if room=rm_characterselect
	...
	key_jump = keyboard_check_pressed(ord(global.ControlJump[_controlno])) or gamepad_button_check_pressed(i,global.gpControlJump[_controlno])
	
		key_jump_hold = keyboard_check(ord(global.ControlJump[_controlno])) or gamepad_button_check(i,global.gpControlJump[_controlno])
