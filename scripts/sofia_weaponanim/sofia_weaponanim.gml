...
frame_set(4,3,0.1)
	...
	flashFX(x+42*image_xscale,y,z-57+13,weaponProjFlashSpr,0,1,0,1,1,c_white,1)
...
{
sprite_index=spr_sofia_whip
if AnimFrame=clamp(AnimFrame,0,0.9)
weaponanim(weaponspr,image_index+1,9,-68-03,0,weaponcolor)
if AnimFrame=clamp(AnimFrame,1,1.9)
weaponanim(weaponspr,image_index+1,14,-52-03,0,weaponcolor)
if AnimFrame=clamp(AnimFrame,2,2.9)
weaponanim(weaponspr,image_index+1,11,-52-03,0,weaponcolor)
if AnimFrame=clamp(AnimFrame,3,3.9)
weaponanim(weaponspr,image_index+1,13,-52-03,0,weaponcolor)
if AnimFrame=clamp(AnimFrame,4,4.9)
weaponanim(weaponspr,image_index+1,13,-52-03,0,weaponcolor)
...
if AnimFrame>6.7 {canmove=1 anim=0}
	...
	PlaySoundNoStack(weaponProjSnd)
	flashFX(x+42*image_xscale,y,z-57+13,spr_icelaserfx,0,0.25,0,image_xscale,1,c_white,1)}
	sprite_index=spr_sofia_gunstand
	atkcol_set(207,0,48-13,9.65,1,17) 
	...
	{ atk=1
	PlaySoundNoStack(weaponProjSnd)
	flashFX(x+42*image_xscale,y,z-57+13,weaponProjFlashSpr,0,1,0,image_xscale,1,c_white,1)
	}
	if AnimFrame=0.25
	{
	projectile_create(x+42*image_xscale,y,z-57+13,-8,weaponProjSpr,weaponProjSpd*image_xscale,weaponProjMask,weaponProjHitSpr,weaponDamage,weaponHitType,weapontargetHeight,0,0)
...
if AnimFrame=clamp(AnimFrame,2,2.9) weaponBack=0 else weaponBack=1
	...
	if AnimFrame=clamp(AnimFrame,0,0.9)
	{weaponanim(weaponspr,weaponIndex,9,-68+3,90*image_xscale,weaponcolor)}
	if AnimFrame=clamp(AnimFrame,1,1.9)
	{weaponanim(weaponspr,weaponIndex,-28,-82+3,129*image_xscale,weaponcolor)}
	if AnimFrame=clamp(AnimFrame,2,2.9)
	{weaponanim(weaponspr,weaponIndex,-23,-82+3,102*image_xscale,weaponcolor)}
	if AnimFrame=clamp(AnimFrame,3,3.9)
	{weaponanim(weaponspr,weaponIndex,54,-51+3,0*image_xscale,weaponcolor)}
	if AnimFrame=clamp(AnimFrame,4,4.9)
	{weaponanim(weaponspr,weaponIndex,53,-47+3,0*image_xscale,weaponcolor)}
	if AnimFrame=clamp(AnimFrame,5,5.9)
	{weaponanim(weaponspr,weaponIndex,25,-66+3,111*image_xscale,weaponcolor)}
	sprite_index=spr_sofia_whip
	image_speed=0
	if AnimFrame=clamp(AnimFrame,3,3.9) atk=1 else atk=0
	if AnimFrame>5.5 {hurt=0 atk=0 canmove=1 hit=0}
...
with flame	{z=-57+13 image_speed=0.5 sprite_index=spr_hina_firebreath alarm[0]=0}
...
with flame	{z=-57+13 image_speed=0.5 sprite_index=spr_hina_firebreath alarm[0]=0}
		 ...
		 {
		flame=instance_create_depth(x+38*image_xscale,y,depth,oFlashFX) flame.image_xscale=image_xscale
with flame	{z=-57+13 image_speed=0.5 sprite_index=spr_hina_firebreath2 alarm[0]=0}
		 }
	frame_set(2,0,0.1)
	frame_set(3,0,0.1) 
...
///Ice Gun
	...
	frame_set(1,1+specialtimes[6],0.025) if AnimFrame=clamp(AnimFrame,1,2) {atk=1
		if specialtimes[5]=0 {specialtimes[5]=1 
flame=instance_create_depth(x+38*image_xscale,y,depth,oFlashFX) flame.image_xscale=image_xscale
with flame	{z=-57+13 image_speed=0.5 sprite_index=spr_robot_ice alarm[0]=0}
	} specialtimes[5]=1 if !instance_exists(flame) 	
	{
	flame=instance_create_depth(x+38*image_xscale,y,depth,oFlashFX) flame.image_xscale=image_xscale
with flame	{z=-57+13 image_speed=0.5 sprite_index=spr_robot_ice image_index=4 alarm[0]=0}
	}}		
		 else atk=0
		 if AnimFrame=2
		 {
		flame=instance_create_depth(x+38*image_xscale,y,depth,oFlashFX) flame.image_xscale=image_xscale
with flame	{z=-57+13 image_speed=0 sprite_index=spr_robot_ice image_index=2 alarm[0]=5}
		 }
	frame_set(2,0,0.1)
	frame_set(3,0,0.1) 
