...
if frameInd>15.9 frameInd=0

if frameInd=clamp(frameInd,4,12) {paintingScale=0 frameTotal=frameInd}
else
...
else if frameInd=clamp(frameInd,8,8.9) {frameTotal=8 paintingScale=-1.00}
...
else if frameInd=clamp(frameInd,13,13.9)  {frameTotal=13 paintingScale=0.75}
else if frameInd=clamp(frameInd,14,14.9)  {frameTotal=14 paintingScale=0.5}
else if frameInd=clamp(frameInd,15,5.9)  {frameTotal=15 paintingScale=0.25}
...
part.z-=48
part.image_angle=random(360) part.sprite_index=spr_dust
if iceFX <2
