...
{   //We are overriding the palette for fading.  Let's build a custom palette.
    if(!surface_exists(override_surface))
        override_surface=surface_create(3,sprite_get_height(my_pal_sprite)); //Create the override palette surface if it doesn't exist.
    surface_set_target(override_surface);
    {   //Build Palette
        draw_sprite(my_pal_sprite,0,0,0); //Draw Base Palette to the surface
        //Set current palette as palette "1";
        pal_swap_draw_palette(my_pal_sprite,current_pal,1,0);
        //Build Custom palette as palette "2";
        draw_set_color(c_white);
        draw_line(2,-1,2,sprite_get_height(my_pal_sprite)); //Just drawing a line of white vertically down the surface.
    }
