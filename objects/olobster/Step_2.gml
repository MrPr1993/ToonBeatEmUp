...
{
if distance_to_point(targetEnemy.x,targetEnemy.y)>90
anim=13 else
anim=11
}
else
...
frame_set(0,0,0.1) damage=0.05 HitForce=-1
if specialcheck5=0 MoveType=1 else MoveType=0
frame_set(1,1,0.01) if animFrame=clamp(animFrame,3,5.9) {atk=1 sentflying=6*image_xscale} else {atk=0 sentflying=0}
frame_set(2,2,0.1) if animFrame=3 PlaySoundNoStack(snd_swing)
frame_set(3,3,0.5)
frame_set(4,3,0.5)
frame_set(5,3,0.25) if animFrame>5.75 if specialcheck5!=0 {specialcheck5-=1 animFrame=2}
