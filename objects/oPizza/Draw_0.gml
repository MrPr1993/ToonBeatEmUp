...
pal_swap_set(weapon_pal_sprite,weapon_pal,false);
if itemCentered=0
draw_sprite_ext(altspr,altind,round(x-lengthdir_x(sprite_get_bbox_right(altspr)/2,angleImage)*image_xscale),round(y-lengthdir_y(sprite_get_bbox_bottom(altspr)/2,angleImage)+z+trainz+zAdd*zAddGround),image_xscale,image_yscale,(angleImage+angleFX+(groundRot*ground))*image_xscale,image_blend,image_alpha)
else
draw_sprite_ext(altspr,altind,round(x),round(y+trainz+z+zAdd*zAddGround),image_xscale,image_yscale,(angleImage+angleFX+(groundRot*ground))*image_xscale,image_blend,image_alpha)
